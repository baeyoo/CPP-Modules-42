...
Form::Form():_name("default_form"), _signvalue(50), _execvalue(50)
{
    _signed = false;
...
std::ostream& operator<<(std::ostream& out, Form & form)
{
    ...
    << "Form target:\t" << form.getTarget() << std::endl
	<< "---------------------------";
    return(out);
}
...
{
    if (!_signed)
        throw FormException("Form is not signed");
    if (executor.getGrade() > getExecValue())
        throw GradeTooLowException(executor.getName() + " level is too low");
}